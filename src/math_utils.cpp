...
double computeInliers(const vector<Vector3d>& features1,  // c1
  ...
  for (size_t j = 0; j < features1.size(); ++j) {
    double dr = 0, dc = 0;
    // depthFromTriangulationExact(R,t,features1[j],features2[j],dc,dr);
    // Vector3d temp = features1[j]*dc;
    xyz_vec.push_back(
        triangulateFeatureNonLin(R, t, features1[j], features2[j]));

    double e1 = reprojError(features1[j], xyz_vec.back(), error_multiplier2);
    double e2 = reprojError(features2[j], R.transpose() * (xyz_vec.back() - t),
                            error_multiplier2);
...
void computeInliersOneView(const vector<Vector3d>& feature_sphere_vec,
                           ...
                           vector<int>& outliers) {
  inliers.clear();
  inliers.reserve(xyz_vec.size());
  outliers.clear();
  outliers.reserve(xyz_vec.size());
  for (size_t j = 0; j < xyz_vec.size(); j++) {
    double e = reprojError(feature_sphere_vec[j],
                           R.transpose() * (xyz_vec[j] - t), error_multiplier2);
    if (e < reproj_thresh)
      inliers.push_back(j);
    else
      outliers.push_back(j);
...
Matrix3d angax2dcm(const Vector3d& n, const double& angle) {
  ...
  return Matrix3d(Matrix3d::Identity() + sqewn * sin(angle) +
                  sqewn * sqewn * (1 - cos(angle)));
}

double sampsonusError(const Vector2d& v2Dash, const Matrix3d& Essential,
                      const Vector2d& v2) {
