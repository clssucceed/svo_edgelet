...
void Frame::initFrame(const cv::Mat& img) {
  ...
  if (img.empty() || img.type() != CV_8UC1 || img.cols != cam_->width() ||
    ...
    throw std::runtime_error(
        "Frame: provided image has not the same size as the camera model or "
  ...
  // Build Image Pyramid
  frame_utils::createImgPyramid(
      img, max(Config::nPyrLevels(), Config::kltMaxLevel() + 1), img_pyr_);
}
...
void Frame::addFeature(Feature* ftr) { fts_.push_back(ftr); }
...
bool Frame::isVisible(const Vector3d& xyz_w) const {
  ...
  if (px[0] >= 0.0 && px[1] >= 0.0 && px[0] < cam_->width() &&
      px[1] < cam_->height())
    return true;
  return false;
}
...
void halfSampleNEON(const cv::Mat& in, cv::Mat& out) {
  for (int y = 0; y < in.rows; y += 2) {
    ...
    for (int x = in.cols; x > 0;
         x -= 16, in_top += 16, in_bottom += 16, out_data += 8) {
...
/*
#ifdef __SSE2__
if(aligned_mem::is_aligned16(in.data) && aligned_mem::is_aligned16(out.data) &&
((in.cols % 16) == 0))
{
  ...
  while (bottom < end) {
    for (int j = 0; j < out_width; j++) {
      *p = static_cast<uint8_t>(
          (uint16_t(top[0]) + top[1] + bottom[0] + bottom[1]) / 4);
      p++;
      top += 2;
      bottom += 2;
    }
