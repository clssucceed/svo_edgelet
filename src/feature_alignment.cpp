...
bool align2D(const cv::Mat& cur_img, uint8_t* ref_patch_with_border,
  ...
  #ifdef __SSE2__
  return
  align2D_SSE2(cur_img,ref_patch_with_border,ref_patch,n_iter,cur_px_estimate);
  #endif
  */
  const int halfpatch_size_ = 4;
...
#define DESCALE(x, n) \
  (((x) + (1 << ((n)-1))) >> (n))  // rounds to closest integer and descales
...
bool align2D_SSE2(const cv::Mat& cur_img, uint8_t* ref_patch_with_border,
  ...
  const int cur_step = cur_img.step.p[0];
  const float Dinv =
      1.0f / (A11 * A22 - A12 * A12);  // we are missing an extra factor 16
  float chi2 = 0;
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    int u_r = floor(u);
    int v_r = floor(v);
    if (u_r < halfpatch_size || v_r < halfpatch_size ||
        u_r >= cur_img.cols - halfpatch_size ||
    ...
    int wTL =
        static_cast<int>((1.0f - subpix_x) * (1.0f - subpix_y) * (1 << W_BITS));
    ...
    __m128i qw0 = _mm_set1_epi32(
        wTL +
        (wTR << 16));  // Sets the 4 signed 32-bit integer values to [wTL, wTR].
    ...
    for (int y = 0; y < patch_size; ++y) {
      const uint8_t* it = (const uint8_t*)cur_img.data +
                          (v_r + y - halfpatch_size) * cur_step + u_r -
                          halfpatch_size;

      // Iptr is aligned!
      //__m128i diff0 = _mm_load_si128((const __m128i*)(ref_patch + y*8));
      __m128i diff = _mm_unpacklo_epi8(
          _mm_loadl_epi64((const __m128i*)(ref_patch + y * 8)), z);

      // load the lower 64 bits and unpack [8u 0 8u 0..]
      __m128i v00 = _mm_unpacklo_epi8(_mm_loadl_epi64((const __m128i*)(it)), z);
      __m128i v01 =
          _mm_unpacklo_epi8(_mm_loadl_epi64((const __m128i*)(it + 1)), z);
      __m128i v10 = _mm_unpacklo_epi8(
          _mm_loadl_epi64((const __m128i*)(it + cur_step)), z);
      __m128i v11 = _mm_unpacklo_epi8(
          _mm_loadl_epi64((const __m128i*)(it + cur_step + 1)), z);

      // interpolate top row and bottom row
      // _mm_unpacklo_epi16: Interleaves the lower 4 signed or unsigned 16-bit
      // integers in a with the lower 4 signed or unsigned 16-bit integers in b.
      // _mm_madd_epi16: Multiplies the 8 signed 16-bit integers from a by the 8
      // signed 16-bit integers from b.
      __m128i t0 =
          _mm_add_epi32(_mm_madd_epi16(_mm_unpacklo_epi16(v00, v01), qw0),
                        _mm_madd_epi16(_mm_unpacklo_epi16(v10, v11), qw1));
      __m128i t1 =
          _mm_add_epi32(_mm_madd_epi16(_mm_unpackhi_epi16(v00, v01), qw0),
                        _mm_madd_epi16(_mm_unpackhi_epi16(v10, v11), qw1));

      // _mm_srai_epi32: Shifts the 4 signed 32-bit integers in A right by count
      // bits while shifting in the sign bit.
      t0 = _mm_srai_epi32(_mm_add_epi32(t0, qdelta),
                          W_BITS);  // adding qdelta is for rounding closest int
      t1 = _mm_srai_epi32(_mm_add_epi32(t1, qdelta), W_BITS);

      // compute 8xres:
      // _mm_packs_epi32: Packs the 8 signed 32-bit integers from a and b into
      // signed 16-bit integers and saturates.
      diff = _mm_subs_epi16(_mm_packs_epi32(t0, t1), diff);

      // load gradient dX and dY, both are aligned!
      v00 = _mm_load_si128(
          (const __m128i*)(ref_patch_dx +
                           y * patch_size));  // [dx1, dx2, dx3, dx4 ...]
      v01 = _mm_load_si128(
          (const __m128i*)(ref_patch_dy +
                           y * patch_size));  // [dy1, dy2, dy3, dy4 ...]

      // _mm_mulhi_epi16: Multiplies the 8 signed 16-bit integers from a by the
      // 8 signed 16-bit integers from b.
      v10 = _mm_mullo_epi16(v00, diff);  // Packs the lower 16 bits of the 8
                                         // signed 32-bit results. [15:0]
      v11 = _mm_mulhi_epi16(v00, diff);  // Packs the upper 16 bits of the 8
                                         // signed 32-bit results. [31:16]

      // _mm_unpacklo_epi16: Interleaves the lower 4 signed or unsigned 16-bit
      // integers with the lower 4 signed or unsigned 16-bit integers in b.
    ...
    // compute -A^-1*b
    update_u = ((A12 * b2 - A22 * b1) * Dinv) * 2;  // * 2 to compensate because
                                                    // above, we did not compute
...
bool align2D_NEON(const cv::Mat& cur_img, uint8_t* ref_patch_with_border,
  ...
  for (int y = 0; y < patch_size; ++y) {
    ...
    for (int x = 0; x < patch_size; ++x, ++it, ++it_dx, ++it_dy) {
      ...
      *it_dy = static_cast<int16_t>(it[ref_step] -
                                    it[-ref_step]);  // divide by 2 missing
  ...
  for (int iter = 0; iter < n_iter; ++iter) {
    ...
    const uint16_t wTL = static_cast<uint16_t>(
        (1.0f - subpix_x) * (1.0f - subpix_y) * (1 << SHIFT_BITS));
    const uint16_t wTR =
        static_cast<uint16_t>(subpix_x * (1.0f - subpix_y) * (1 << SHIFT_BITS));
    const uint16_t wBL =
        static_cast<uint16_t>((1.0f - subpix_x) * subpix_y * (1 << SHIFT_BITS));
    const uint16_t wBR =
        static_cast<uint16_t>((1 << SHIFT_BITS) - wTL - wTR - wBL);
    ...
    for (int y = 0; y < patch_size; ++y) {
      ...
      int16x8_t res = vsubq_s16(
          vreinterpretq_s16_u16(v00),
          vreinterpretq_s16_u16(vmovl_u8(vld1_u8(ref_patch + y * 8))));
    ...
    update = Hinv * Jres * 2;  // * 2 to compensate because above, we did not
                               // compute the derivative correctly
    u += update[0];
    v += update[1];
    mean_diff += update[2];
