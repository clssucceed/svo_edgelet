...
InitResult KltHomographyInit::addFirstFrame(FramePtr frame_ref) {
  ...
  if (px_ref_.size() < 100) {
    SVO_WARN_STREAM_THROTTLE(2.0,
                             "First image has less than 100 features. Retry in "
                             "more textured environment.");
    return FAILURE;
  }
  ...
  for (size_t i = 0, i_max = px_ref_.size(); i < i_max; ++i) {
    if (px_ref_[i].x > 640 / 4 && px_ref_[i].x < 640 * 3 / 4 &&
        px_ref_[i].y > 480 / 4 && px_ref_[i].y < 480 * 3 / 4) {
      fts_center_img++;
    }
  }
  if (fts_center_img < 50) {
    SVO_WARN_STREAM_THROTTLE(2.0,
                             "First image,  features in center img are less. "
  ...
  img_prev_ =
      frame_ref_->img_pyr_[0]
          .clone();  // init img_prev_:  used to track feature frame by frame
  px_prev_ = px_ref_;
...
InitResult KltHomographyInit::addSecondFrame(FramePtr frame_cur) {
  ...
  // disparities_);
  trackKlt(frame_ref_, frame_cur, fts_type_, px_ref_, px_cur_, f_ref_, f_cur_,
           disparities_, img_prev_, px_prev_);
  ...
  if (disparity < Config::initMinDisparity()) return NO_KEYFRAME;

  double sum = std::accumulate(disparities_.begin(), disparities_.end(), 0.0);
  double mean = sum / disparities_.size();
  double sq_sum = std::inner_product(disparities_.begin(), disparities_.end(),
                                     disparities_.begin(), 0.0);
  ...
  computeHomography(f_ref_, f_cur_, frame_ref_->cam_->errorMultiplier2(),
                    Config::poseOptimThresh(), inliers_, xyz_in_cur_,
                    T_cur_from_ref_, method_flag);
  SVO_INFO_STREAM("Init: Homography RANSAC " << inliers_.size() << " inliers.");

  if (inliers_.size() < Config::initMinInliers()) {
    SVO_WARN_STREAM("Init WARNING: " << Config::initMinInliers()
                                     << " inliers minimum required.");
    return FAILURE;
  }
  ...
  for (vector<int>::iterator it = inliers_.begin(); it != inliers_.end();
       ++it) {
    ...
    if (frame_ref_->cam_->isInFrame(px_cur.cast<int>(), 10) &&
      ...
      if (fts_type[2] == 0) {
        ...
        Feature* ftr_ref(
            new Feature(frame_ref_.get(), new_point, px_ref, f_ref_[*it], 0));
        frame_ref_->addFeature(ftr_ref);
        new_point->addFrameRef(ftr_ref);
      } else {
        Vector2d g(fts_type[0], fts_type[1]);
        Feature* ftr_cur(
            new Feature(frame_cur.get(), new_point, px_cur, f_cur_[*it], g, 0));
        frame_cur->addFeature(ftr_cur);
        new_point->addFrameRef(ftr_cur);

        Feature* ftr_ref(new Feature(frame_ref_.get(), new_point, px_ref,
                                     f_ref_[*it], g, 0));
        frame_ref_->addFeature(ftr_ref);
        new_point->addFrameRef(ftr_ref);
      }
...
klt_max_iter, klt_eps);
  ...
  for(size_t i=0; px_ref_it != px_ref.end(); ++i)
  {
    ...
    disparities.push_back(Vector2d(px_ref_it->x - px_cur_it->x, px_ref_it->y -
px_cur_it->y).norm());
    ++px_ref_it;
    ++px_cur_it;
    ++f_ref_it;
...
void trackKlt(FramePtr frame_ref, FramePtr frame_cur,
              vector<Vector3d>& fts_type, vector<cv::Point2f>& px_ref,
              vector<cv::Point2f>& px_cur, vector<Vector3d>& f_ref,
              vector<Vector3d>& f_cur, vector<double>& disparities,
              cv::Mat& img_prev, vector<cv::Point2f>& px_prev) {
  const double klt_win_size = 30.0;  // 30
  const int klt_max_iter = 30;
  const double klt_eps = 0.001;
  vector<uchar> status;
  ...
  cv::calcOpticalFlowPyrLK(img_prev, frame_cur->img_pyr_[0], px_prev, px_cur,
                           status, error,
                           cv::Size2i(klt_win_size, klt_win_size), 3, termcrit,
  ...
  for (size_t i = 0; px_ref_it != px_ref.end(); ++i) {
    ...
    disparities.push_back(
        Vector2d(px_ref_it->x - px_cur_it->x, px_ref_it->y - px_cur_it->y)
            .norm());
    ++fts_type_it;
    ++px_ref_it;
    ++px_cur_it;
...
void computeHomography(const vector<Vector3d>& f_ref,
                       const vector<Vector3d>& f_cur, double focal_length,
                       double reprojection_threshold, vector<int>& inliers,
  ...
  if (2 == method_choose)  //  E method
  {
    ...
    */
    ...
    cv::Point2d pp(0, 0);
    double focal = 1;
    cv::Mat E = svo::findEssentialMat(x1, x2, focal, pp, CV_RANSAC, 0.99,
                                      2.0 / focal_length, cv::noArray());
    std::cout << "=======================findEssentialMat======================"
                 "========"
              << std::endl;
    cv::Mat R_cf, t_cf;
    ...
    svo::computeInliers(f_cur, f_ref, R, t, reprojection_threshold,
                        focal_length, xyz_in_cur, inliers, outliers);

    SVO_INFO_STREAM("Init: findEssentialMat RANSAC " << inliers.size()
                                                     << " inliers.");
    ...
    for (vector<int>::iterator it = inliers.begin(); it != inliers.end();
         ++it) {
      if (xyz_in_cur[*it].z() < 0) depth_error++;
    }
    if (depth_error > 4) method_choose = 1;
/*
...
*/
    ...
    cv::Point2d pp(0, 0);
    double focal = 1;
    std::cout << "=======================findFundamentalMat===================="
                 "=========="
              << std::endl;
    cv::Mat F = cv::findFundamentalMat(x1, x2, FM_RANSAC, 2.0 / focal_length,
                                       0.99, noArray());
    ...
    SVO_INFO_STREAM("Init: findFundamentalMat  RANSAC " << inliers.size()
                                                        << " inliers.");
    // std::cout<<"R: "<<R_cf<<std::endl;
    // std::cout<<"t: "<<t_cf<<std::endl;
    std::cout << R << std::endl << t << std::endl;
  ...
  if (1 == method_choose) {
    ...
    svo::Homography Homography(uv_ref, uv_cur, focal_length,
                               reprojection_threshold);
    Homography.computeSE3fromMatches();
    svo::computeInliers(f_cur, f_ref, Homography.T_c2_from_c1.rotation_matrix(),
                        Homography.T_c2_from_c1.translation(),
                        reprojection_threshold, focal_length, xyz_in_cur,
                        inliers, outliers);
    ...
    */
    ...
    std::cout << "T_cur_from_ref: " << T_cur_from_ref << std::endl;
    SVO_INFO_STREAM("Init: Homography RANSAC " << inliers.size()
                                               << " inliers.");
