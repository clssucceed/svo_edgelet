...
void Point::initNormal() {
  ...
  normal_information_ = DiagonalMatrix<double, 3, 3>(
      pow(20 / (pos_ - ftr->frame->pos()).norm(), 2), 1.0, 1.0);
  normal_set_ = true;
}
...
bool Point::getCloseViewObs(const Vector3d& framepos, Feature*& ftr) const {
  ...
  ftr = *min_it;
  if (min_cos_angle <
      0.5)  // assume that observations larger than 60° are useless
    return false;
  return true;
}
/*
