...
void Viewer::DrawKeyFrames(const bool bDrawKF) {
  ...
  */
  ...
  for (size_t i = 0; i < _pos.size(); i++) {
    ...
    glVertex3d(Twc.translation()[0], Twc.translation()[1],
               Twc.translation()[2]);
  }
  glEnd();
}
...
void Viewer::DrawMapRegionPoints() {
  ...
  std::vector<std::pair<svo::FramePtr, size_t> > overlap_kfs =
      _vo->overlap_kfs();
  auto it_frame = overlap_kfs.begin();
  for (size_t i = 0; i < overlap_kfs.size(); ++i, ++it_frame) {
    svo::FramePtr frame = it_frame->first;
    for (svo::Features::iterator it = frame->fts_.begin();
         it != frame->fts_.end(); ++it) {
...
void Viewer::DrawMapSeeds() {
  ...
  glBegin(GL_LINES);
  for (std::list<svo::Seed>::iterator it = seeds.begin(); it != seeds.end();
       ++it) {
    Eigen::Vector3d xyz_min(
        it->ftr->frame->T_f_w_.inverse() *
        (it->ftr->f * (1.0 / (it->mu + std::sqrt(it->sigma2)))));
    float d = std::max(it->mu - std::sqrt(it->sigma2), 0.000001f);
    Eigen::Vector3d xyz_max(it->ftr->frame->T_f_w_.inverse() *
                            (it->ftr->f * (1.0 / d)));
  ...
  for (std::list<svo::Seed>::iterator it = seeds.begin(); it != seeds.end();
       ++it) {
    Eigen::Vector3d xyz_mu(it->ftr->frame->T_f_w_.inverse() *
                           (it->ftr->f * (1.0 / (it->mu))));
