...
SparseAlign::SparseAlign(int max_level, int min_level, int n_iter, bool display,
                         bool verbose)
...
size_t SparseAlign::run(FramePtr ref_frame, FramePtr cur_frame) {
  ...
  visible_fts_.resize(
      ref_patch_cache_.rows,
      false);  // TODO: should it be reset at each level?   // n x 1
...
double SparseAlign::computeResiduals(const SE3& T_cur_from_ref,
  ...
  if (have_ref_patch_cache_ == false) precomputeReferencePatches();

  // compute the weights on the first iteration
  std::vector<float> errors;
  if (compute_weight_scale) errors.reserve(visible_fts_.size());
  ...
  size_t feature_counter =
      0;  // is used to compute the index of the cached jacobian
  std::vector<bool>::iterator visiblity_it = visible_fts_.begin();
  for (auto it = ref_frame_->fts_.begin(); it != ref_frame_->fts_.end();
    ...
    if (u_cur_i < 0 || v_cur_i < 0 || u_cur_i - border < 0 ||
        v_cur_i - border < 0 || u_cur_i + border >= cur_img.cols ||
    ...
    for (int y = 0; y < patch_size_; ++y) {
      ...
      for (int x = 0; x < patch_size_;
        ...
        const float intensity_cur =
            w_cur_tl * cur_img_ptr[0] + w_cur_tr * cur_img_ptr[1] +
        ...
        if (linearize_system) {
          // compute Jacobian, weighted Hessian and weighted "steepest descend
          // images" (times error)
...
int SparseAlign::solve() {
  ...
  if ((bool)std::isnan((double)x_[0])) return 0;
  return 1;
}

void SparseAlign::update(const SE3& T_curold_from_ref, SE3& T_curnew_from_ref) {
  ...
  for (iter_ = 0; iter_ < n_iter_; ++iter_) {
    ...
    if ((iter_ > 0 && new_chi2 > chi2_) || stop_) {
      if (verbose_) {
        std::cout << "It. " << iter_ << "\t Failure"
                  << "\t new_chi2 = " << new_chi2
                  << "\t Error increased. Stop optimizing." << std::endl;
      }
    ...
    if (verbose_) {
      std::cout << "It. " << iter_ << "\t Success"
                ...
                << "\t x_norm = " << norm_max(x_) << std::endl;
    }

    finishIteration();

    // stop when converged, i.e. update step too small
    if (norm_max(x_) <= eps_) break;
  }
}
