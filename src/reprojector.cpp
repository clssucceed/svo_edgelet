...
Reprojector::Reprojector(svo::AbstractCamera* cam, Map& map) : map_(map) {
  initializeGrid(cam);
}

Reprojector::~Reprojector() {
  std::for_each(grid_.cells.begin(), grid_.cells.end(),
                [&](Cell* c) { delete c; });
}
...
void Reprojector::initializeGrid(svo::AbstractCamera* cam) {
  ...
  grid_.grid_n_rows =
      ceil(static_cast<double>(cam->height()) / grid_.cell_size);
  grid_.cells.resize(grid_.grid_n_cols * grid_.grid_n_rows);
  std::for_each(grid_.cells.begin(), grid_.cells.end(),
                [&](Cell*& c) { c = new Cell; });
  grid_.cell_order.resize(grid_.cells.size());
  for (size_t i = 0; i < grid_.cells.size(); ++i) grid_.cell_order[i] = i;
  random_shuffle(
      grid_.cell_order.begin(),
      grid_.cell_order.end());  // maybe we should do it at every iteration!
}
...
void Reprojector::reprojectMap(
  ...
  for (auto it_frame = close_kfs.begin(), ite_frame = close_kfs.end();
    ...
    ref_frame->debug_img_ =
        ref_frame->img().clone();  // every reproject iteration, the debug_img_
                                   // should be reinition
  ...
  // std::cout<<"\033[1;31m"<< "candidates size:
  ...
  for (size_t i = 0; i < grid_.cells.size(); ++i) {
    ...
    if (n_matches_ > (size_t)Config::maxFts()) break;
  }
...
bool Reprojector::pointQualityComparator(Candidate& lhs, Candidate& rhs) {
  if (lhs.pt->type_ > rhs.pt->type_) return true;
  return false;
}
...
bool Reprojector::reprojectCell(Cell& cell, FramePtr frame) {
  ...
  while (it != cell.end()) {
    ...
    if (!found_match) {
      /*
              //debug:  show not match feature
               if( matcher_.ref_ftr_->type == Feature::EDGELET)
                 cv::line(matcher_.ref_ftr_->frame->debug_img_,
                          cv::Point2f( matcher_.ref_ftr_->px[0]+3*
         matcher_.ref_ftr_->grad[1], matcher_.ref_ftr_->px[1]-3*
         matcher_.ref_ftr_->grad[0]),
                          cv::Point2f( matcher_.ref_ftr_->px[0]-3*
         matcher_.ref_ftr_->grad[1],  matcher_.ref_ftr_->px[1]+3*
         matcher_.ref_ftr_->grad[0]),
                          cv::Scalar(255), 2);
               else
                 cv::rectangle(matcher_.ref_ftr_->frame->debug_img_,
                               cv::Point2f( matcher_.ref_ftr_->px[0]-5,
         matcher_.ref_ftr_->px[1]-5),
                               cv::Point2f( matcher_.ref_ftr_->px[0]+5,
         matcher_.ref_ftr_->px[1]+5),
               ...
               cv::rectangle(img,
                             cv::Point2f(it->px[0]-5, it->px[1]-5),
                             cv::Point2f( it->px[0]+5,  it->px[1]+5),
                             cv::Scalar(255));

               std::stringstream s1,s2;
               s1<<"frame_"<<frame->id_;
               cv::imshow(s1.str().c_str(),img);
              s2<<"match_"<<matcher_.ref_ftr_->frame->id_;

               cv::imshow(s2.str().c_str(),matcher_.ref_ftr_->frame->debug_img_);
               cv::waitKey(0);
               // end debug
      */
      ...
      if (it->pt->type_ == Point::TYPE_CANDIDATE &&
          it->pt->n_failed_reproj_ > 30)
    ...
    if (it->pt->type_ == Point::TYPE_UNKNOWN &&
        it->pt->n_succeeded_reproj_ > 10)
      it->pt->type_ = Point::TYPE_GOOD;

    Feature* new_feature =
        new Feature(frame.get(), it->px, matcher_.search_level_);
...
bool Reprojector::reprojectPoint(FramePtr frame, Point* point) {
  ...
  if (frame->cam_->isInFrame(px.cast<int>(),
                             8))  // 8px is the patch size in the matcher
  {
    const int k =
        static_cast<int>(px[1] / grid_.cell_size) * grid_.grid_n_cols +
