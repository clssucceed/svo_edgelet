#include <svo/corner_10.h>
#include <svo/fast.h>
#include <svo/faster_corner_utilities.h>
#include <list>
#include <vector>
...
void faster_corner_detect_10(const fast_byte* img, int img_width,
                             int img_height, int img_stride, short barrier,
  ...
  for (int y = 3; y < img_height - 3; y++) {
    for (int x = 3; x < 16; x++)
      if (is_corner_10<Less>((fast_byte*)img + y * img_stride + x, img_stride,
                             barrier) ||
          is_corner_10<Greater>((fast_byte*)img + y * img_stride + x,
                                img_stride, barrier))
    ...
    for (int x = 16; x < xend; x += 16) {
      ...
      unsigned int ans_a, ans_c;
      {
        ...
        __m128i c = _mm_insert_epi16(
            _mm_srli_si128(a, 2), *(const unsigned short*)(p + 15 - stride), 7);
      ...
      unsigned int ans_d, ans_f;
      {
        ...
        __m128i f = _mm_insert_epi16(
            _mm_srli_si128(d, 2), *(const unsigned short*)(p + 15 + stride), 7);
      ...
      {
        if (possible & (1 << 0)) corners.push_back(fast_xy(x + 0, y));
        if (possible & (1 << 1)) corners.push_back(fast_xy(x + 1, y));
        if (possible & (1 << 2)) corners.push_back(fast_xy(x + 2, y));
        if (possible & (1 << 3)) corners.push_back(fast_xy(x + 3, y));
        if (possible & (1 << 4)) corners.push_back(fast_xy(x + 4, y));
        if (possible & (1 << 5)) corners.push_back(fast_xy(x + 5, y));
        if (possible & (1 << 6)) corners.push_back(fast_xy(x + 6, y));
        if (possible & (1 << 7)) corners.push_back(fast_xy(x + 7, y));
      }
      // if(possible & 0xf0) //Does this mak( ,  fast)r?
      {
        if (possible & (1 << 8)) corners.push_back(fast_xy(x + 8, y));
        if (possible & (1 << 9)) corners.push_back(fast_xy(x + 9, y));
        if (possible & (1 << 10)) corners.push_back(fast_xy(x + 10, y));
        if (possible & (1 << 11)) corners.push_back(fast_xy(x + 11, y));
        if (possible & (1 << 12)) corners.push_back(fast_xy(x + 12, y));
        if (possible & (1 << 13)) corners.push_back(fast_xy(x + 13, y));
        if (possible & (1 << 14)) corners.push_back(fast_xy(x + 14, y));
        if (possible & (1 << 15)) corners.push_back(fast_xy(x + 15, y));
      }
    }
...
void fast_corner_detect_10_sse2(const fast_byte* img, int img_width,
                                int img_height, int img_stride, short barrier,
                                std::vector<fast_xy>& corners) {
  if (img_width < 22) {
    fast_corner_detect_10(img, img_width, img_height, img_stride, barrier,
                          corners);
    return;
