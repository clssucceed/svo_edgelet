...
void fast_nonmax_3x3(const std::vector<fast_xy>& corners,
  ...
  for (int i = 0; i < sz; i++) {
    ...
    if (pos.y != 0 && row_start[pos.y - 1] != -1) {
      ...
      // Make point_above point to the first of the pixels above the current
      // point,
      // if it exists.
      for (;
      ...
      for (int i = point_above;
           corners[i].y < pos.y && corners[i].x <= pos.x + 1; i++) {
        int x = corners[i].x;
        ...
        point_below < sz)  // Nothing below
    {
      ...
      // Make point below point to one of the pixels belowthe current point, if
      // it
      // exists.
      ...
      for (int i = point_below;
           i < sz && corners[i].y == pos.y + 1 && corners[i].x <= pos.x + 1;
           i++) {
        int x = corners[i].x;
        if ((x == pos.x - 1 || x == pos.x || x == pos.x + 1) &&
            (scores[i] >= score))
