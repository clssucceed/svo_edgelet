...
void Homography::calcFromMatches() {
  ...
  cv::Mat cvH =
      cv::findHomography(src_pts, dst_pts, CV_RANSAC, 2. / error_multiplier2);
...
bool Homography::decompose() {
  ...
  if (nCase != 1) {
    printf(
        "FATAL Homography Initialization: This motion case is not implemented "
        "or is degenerate. Try again. ");
    return false;
  }
