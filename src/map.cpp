...
void Map::getCloseKeyframes(
  ...
  for (auto kf : keyframes_) {
    // check if kf has overlaping field of view with frame, use therefore
    // KeyPoints
    for (auto keypoint : kf->key_pts_) {
      ...
      if (frame->isVisible(keypoint->point->pos_)) {
        ...
        break;  // this keyframe has an overlapping field of view -> add to
                // close_kfs
      }
    }
  }
...
void Map::transform(const Matrix3d& R, const Vector3d& t, const double& s) {
  for (auto it = keyframes_.begin(), ite = keyframes_.end(); it != ite; ++it) {
    ...
    for (auto ftr = (*it)->fts_.begin(); ftr != (*it)->fts_.end(); ++ftr) {
      ...
      if ((*ftr)->point->last_published_ts_ == -1000) continue;
...
MapPointCandidates::~MapPointCandidates() { reset(); }
...
void frameValidation(Frame* frame, int id) {
  for (auto it = frame->fts_.begin(); it != frame->fts_.end(); ++it) {
    ...
    if (!(*it)->point->findFrameRef(frame))
      printf(
          "ERROR DataValidation %i: Frame has reference but point does not "
...
void pointValidation(Point* point, int id) {
  for (auto it = point->obs_.begin(); it != point->obs_.end(); ++it) {
    bool found = false;
    for (auto it_ftr = (*it)->frame->fts_.begin();
         it_ftr != (*it)->frame->fts_.end(); ++it_ftr)
    ...
    if (!found)
      printf(
          "ERROR DataValidation %i: Point %i has inconsistent reference in "
          "frame %i, is candidate = %i\n",
