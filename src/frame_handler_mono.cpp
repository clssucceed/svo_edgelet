...
void FrameHandlerMono::Debug_show_img() {
  ...
  std::cout << "debug show, new frame id : " << new_frame_->id_
            << "last frame id: " << last_frame_->id_ << std::endl;
  for (Features::iterator it = new_frame_->fts_.begin();
    ...
    if ((*it)->type == Feature::EDGELET)
      ...
      cv::rectangle(img_new, cv::Point2f((*it)->px[0] - 2, (*it)->px[1] - 2),
  ...
  for (Features::iterator it = last_frame_->fts_.begin();
    ...
    if ((*it)->type == Feature::EDGELET)
      ...
      cv::rectangle(img_last, cv::Point2f((*it)->px[0] - 2, (*it)->px[1] - 2),
                    cv::Point2f((*it)->px[0] + 2, (*it)->px[1] + 2),
                    cv::Scalar(0, 255, 0), CV_FILLED);
  }
  ...
  std::for_each(
      overlap_kfs_.begin(), overlap_kfs_.end(), [&](pair<FramePtr, size_t>& i) {
        ...
        for (Features::iterator it = fptr->fts_.begin(); it != fptr->fts_.end();
          ...
          if ((*it)->type == Feature::EDGELET)
            ...
            cv::rectangle(img, cv::Point2f((*it)->px[0] - 2, (*it)->px[1] - 2),
                          cv::Point2f((*it)->px[0] + 2, (*it)->px[1] + 2),
                          cv::Scalar(0, 255, 0), CV_FILLED);
        }
        imshow(s, img);
...
void FrameHandlerMono::initialize() {
  ...
  feature_detection::DetectorPtr edge_detector(
      new feature_detection::EdgeDetector(cam_->width(), cam_->height(),
                                          Config::gridSize(),
                                          Config::nPyrLevels()));
  ...
  depth_filter_ =
      new DepthFilter(feature_detector, edge_detector, depth_filter_cb);
  depth_filter_->startThread();
}

FrameHandlerMono::~FrameHandlerMono() { delete depth_filter_; }
...
void FrameHandlerMono::addImage(const cv::Mat& img, const double timestamp) {
  ...
  // debuge
  {
    ...
    if (stage_ == STAGE_SECOND_FRAME) {
      ...
      for (vector<cv::Point2f>::const_iterator it_ref = px_ref.begin(),
                                               it_cur = px_cur.begin();
           it_ref != px_ref.end(); ++it_type, ++it_ref, ++it_cur) {
        if ((*it_type)[2] == 1)
          cv::line(img_new, cv::Point2f(it_cur->x, it_cur->y),
      ...
      for (Features::iterator it = new_frame_->fts_.begin();
        ...
        if ((*it)->type == Feature::EDGELET)
          ...
          cv::rectangle(img_new,
                        cv::Point2f((*it)->px[0] - 2, (*it)->px[1] - 2),
                        cv::Point2f((*it)->px[0] + 2, (*it)->px[1] + 2), c);
      }
    }
    /*
     　//save image
      std::stringstream ss;
      ss<<"img/"<<std::setw( 6 ) << std::setfill( '0'
     )<<new_frame_->id_<<".jpg";
...
FrameHandlerBase::UpdateResult FrameHandlerMono::processSecondFrame() {
  initialization::InitResult res =
      klt_homography_init_.addSecondFrame(new_frame_);
...
#ifdef USE_BUNDLE_ADJUSTMENT
  ba::twoViewBA(new_frame_.get(), map_.lastKeyframe().get(),
                Config::lobaThresh(), &map_);
#endif
...
*/
  SparseAlign img_align(svo::Config::kltMaxLevel(), svo::Config::kltMinLevel(),
                        30, false, false);
  size_t img_align_n_tracked = img_align.run(last_frame_, new_frame_);
  ...
  if (repr_n_new_references < Config::qualityMinFts()) {
    ...
    std::cout << "\033[1;31m"
              << " Not enough matched featrues.  Reproject process failure!"
              << " \033[0m" << std::endl;
    new_frame_->T_f_w_ =
...
// Debug
  ...
  for (Features::iterator it = new_frame_->fts_.begin();
       it != new_frame_->fts_.end(); ++it)
    if ((*it)->point != NULL)
      (*it)->point->addFrameRef(*it);  //  loop , add a link between feature(in
                                       //  new_frame) to  map point, especially,
                                       //  for new candidates point, Here, we
                                       //  link they to feature in new_frame
...
FrameHandlerMono::UpdateResult FrameHandlerMono::relocalizeFrame(
  ...
  */
  ...
  SparseAlign img_align(Config::kltMaxLevel(), Config::kltMinLevel(), 30, false,
                        false);
  size_t img_align_n_tracked = img_align.run(ref_keyframe, new_frame_);
...
bool FrameHandlerMono::relocalizeFrameAtPose(const int keyframe_id,
                                             const SE3& T_f_kf,
                                             const cv::Mat& img,
                                             const double timestamp) {
  FramePtr ref_keyframe;
  if (!map_.getKeyframeById(keyframe_id, ref_keyframe)) return false;
  new_frame_.reset(new Frame(cam_, img.clone(), timestamp));
