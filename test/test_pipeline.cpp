...
BenchmarkNode::BenchmarkNode() {
  ...
  cam_ = new svo::PinholeCamera(640, 480, 520.9, 521.0, 325.1, 249.7, 0.2312,
                                -0.7849, -0.0033, -0.0001, 0.9172);
  // tum rgbd dataset fr1
  // cam_ = new svo::PinholeCamera(640, 480, 517.3,516.5,318.6,	255.3,
  // 0.2624,	-0.9531,-0.0054,0.0026,	1.1633);

  // my camera
  // cam_ = new svo::PinholeCamera(640, 480, 446.867338 ,446.958766, 298.082779,
  // 234.334299, -0.324849, 0.1205156, -0.000186, -0.000821);
  // ICL dataset
  // cam_ = new svo::PinholeCamera(640, 480, 481.20, 480.00, 319.50, 239.50);

  // kitti
  // cam_ = new svo::PinholeCamera(1226, 370, 707.0912, 707.0912, 601.8873,
  // 183.1104);
  // tum mono dataset
  // cam_ = new svo::PinholeCamera(1226, 370, 707.0912, 707.0912, 601.8873,
  // 183.1104);
...
void BenchmarkNode::runFromFolder() {
  ...
  std::vector<double> vTimestamps;
  std::string filepath =
      std::string("/home/dji/data/tum/rgbd_dataset_freiburg2_desk");
  ...
  for (int img_id = 10; img_id < 2700; ++img_id) {
    ...
    ss << "/media/hyj/dataset/datasets/living_room_traj3_frei_png/rgb/"
       << img_id << ".png";  // 26_1, 28_6, 11_3

    if (img_id == 1) std::cout << "reading image " << ss.str() << std::endl;
