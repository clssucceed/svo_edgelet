...
struct Seed {
  ...
  Feature*
      ftr;  //!< Feature in the keyframe for which the depth should be computed.
  float a;  //!< a of Beta distribution: When high, probability of inlier is
            //! large.
...
class DepthFilter {
 public:
  ...
  struct Options {
    bool check_ftr_angle;  //!< gradient features are only updated if the
                           //! epipolar line is orthogonal to the gradient.
    bool epi_search_1d;    //!< restrict Gauss Newton in the epipolar search to
                           //! the epipolar line.
    bool verbose;          //!< display output.
    bool use_photometric_disparity_error;  //!< use photometric disparity error
                                           //! instead of 1px error in tau
    //! computation.
    int max_n_kfs;      //!< maximum number of keyframes for which we maintain
                        //! seeds.
    double sigma_i_sq;  //!< image noise.
    double seed_convergence_sigma2_thresh;  //!< threshold on depth uncertainty
                                            //! for convergence.
    Options()
  ...
  DepthFilter(feature_detection::DetectorPtr feature_detector,
              feature_detection::DetectorPtr edge_detector,
              callback_t seed_converged_cb);

  DepthFilter(feature_detection::PixelSelectorPtr feature_detector,
              callback_t seed_converged_cb);
