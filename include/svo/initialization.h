...
class KltHomographyInit {
 ...
 protected:
  ...
  vector<Vector3d> f_ref_;  //!< bearing vectors corresponding to the keypoints
                            //! in the reference image.
  vector<Vector3d> fts_type_;  // hyj : save feature type
  vector<Vector3d> f_cur_;  //!< bearing vectors corresponding to the keypoints
                            //! in the current image.
  vector<double> disparities_;  //!< disparity between first and second frame.
  vector<int>
      inliers_;  //!< inliers after the geometric check (e.g., Homography).
  vector<Vector3d>
      xyz_in_cur_;      //!< 3D points computed during the geometric check.
  SE3 T_cur_from_ref_;  //!< computed transformation between the first two
                        //! frames.
};
