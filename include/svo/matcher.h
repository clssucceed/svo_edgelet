...
/// Patch-matcher for reprojection-matching and epipolar search in
/// triangulation.
class Matcher {
 public:
  ...
  struct Options {
    ...
    double max_epi_length_optim;  //!< max length of epipolar line to skip
                                  //! epipolar search and directly go to img
    //! align
    size_t max_epi_search_steps;  //!< max number of evaluations along epipolar
                                  //! line
  ...
  /// IMPORTANT! This function assumes that px_cur is already set to an estimate
  /// that is within ~2-3 pixel of the final result!
  ...
  /// features.
  bool findEpipolarMatchDirect(const Frame& ref_frame, const Frame& cur_frame,
