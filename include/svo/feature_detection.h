...
class AbstractDetector {
 public:
  AbstractDetector(const int img_width, const int img_height,
                   const int cell_size, const int n_pyr_levels);

  virtual ~AbstractDetector(){};

  virtual void detect(Frame* frame, const ImgPyr& img_pyr,
                      const double detection_threshold, Features& fts) = 0;
...
class EdgeDetector : public AbstractDetector {
 public:
  EdgeDetector(const int img_width, const int img_height, const int cell_size,
...
class PixelSelector  //: public AbstractDetector
{
 public:
  int gridsize_ = 32;
  int PixelNum_ = 300;
  ...
  int img_height_;
  int n_pyr_levels_;
  cv::Mat occpuancy_;  // flag to show whethe the pixel has depth

  PixelSelector(int img_width, int img_height, int PixelNum, int n_pyr_levels);

  virtual ~PixelSelector() {}

  virtual void detect(Frame* frame, const ImgPyr& img_pyr, Features& fts);

  void setPixelOccpuancy(const Vector2d& px);
  void setExistingFeatures(const Features& fts);
};
typedef boost::shared_ptr<PixelSelector> PixelSelectorPtr;
//
/// FAST detector by Edward Rosten.
class FastDetector : public AbstractDetector {
 public:
  FastDetector(const int img_width, const int img_height, const int cell_size,
               const int n_pyr_levels);
