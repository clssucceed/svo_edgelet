...
class FileReader {
 public:
  ...
  void skipComments() {
    while (file_stream_.good() && file_stream_.peek() == '#') skip(1);
  }
...
void loadBlenderDepthmap(const std::string file_name,
                         const svo::AbstractCamera& cam, cv::Mat& img) {
...
bool getDepthmapNormalAtPoint(const Vector2i& px, const cv::Mat& depth,
                              const int halfpatch_size,
                              const svo::AbstractCamera& cam,
                              Vector3d& normal) {
  assert(cam.width() == depth.cols && cam.height() == depth.rows);
  if (!cam.isInFrame(px, halfpatch_size + 1)) return false;
  ...
  A.resize(n_meas, Eigen::NoChange);
  Matrix<double, Dynamic, 1> b;
  b.resize(n_meas, Eigen::NoChange);
...
std::ostream& operator<<(std::ostream& out, const ImageNameAndPose& gt) {
  out << gt.timestamp_ << " " << gt.image_name_ << " " << gt.t_.x() << " "
      << gt.t_.y() << " " << gt.t_.z() << " " << gt.q_.x() << " " << gt.q_.y()
      << " " << gt.q_.z() << " " << gt.q_.w() << " " << std::endl;
  return out;
}
